...
def load_google_sheet(url):
    """Loads data live from Google Sheets."""
    try:
...
# TABS SETUP
# ==============================================================================
tab_algorithm, tab_form = st.tabs(["🧩 Run Matching Algorithm", "📝 Fill Out Google Form"])
...
with tab_algorithm:
    ...
    # --- LOAD FILES ---
    # 1. Bump Teams (From Google Sheet)
    # You can hardcode the URL here or put it in secrets.toml
    # If in secrets, use: st.secrets["connections"]["gsheets"]["spreadsheet"]
    
    GOOGLE_SHEET_URL = st.text_input("Enter Google Sheet URL for Bump Teams (or leave blank to use Secrets):")
    
    if st.button("🔄 Refresh Data from Google Sheets"):
        st.cache_data.clear()
        
    if GOOGLE_SHEET_URL:
        bump_teams = load_google_sheet(GOOGLE_SHEET_URL)
    else:
        # Fallback to secrets if input is empty
        try:
            bump_teams = st.connection("gsheets", type=GSheetsConnection).read()
        except:
            bump_teams = pd.DataFrame()

    if not bump_teams.empty:
        st.sidebar.success(f"✅ Bump Teams Loaded ({len(bump_teams)} rows)")
    else:
        st.sidebar.warning("⚠️ No Bump Team data found. Check URL or Secrets.")

    # 2. Other Static Files (Local)
    party_excuses = load_excel_safe("Party Excuses for Recruitment - Sorority Members (Responses).xlsx")
    if not party_excuses.empty: st.sidebar.success("✅ Party Excuses Loaded")

    member_interest = load_excel_safe("Sorority Members Interests (Real Data).xlsx")
    if not member_interest.empty: st.sidebar.success("✅ Member Interests Loaded")

    member_pnm_no_match = load_excel_safe("Prior PNM Connection Form - Sorority Members (Responses).xlsx")
    if not member_pnm_no_match.empty: st.sidebar.success("✅ No Match List Loaded")
    ...
    if st.button("Run Matching Algorithm"):
        if pnm_intial_interest.empty or bump_teams.empty:
            st.error("Missing critical data files.")
            st.stop()
...
# ==============================================================================
with tab_form:
    st.header("📝 Fill Out Google Form")
    
    # URL to the Public Google Form
    google_form_url = st.text_input("Enter the Shareable Link to your Google Form:", placeholder="https://docs.google.com/forms/d/e/...")
    
    if google_form_url:
        st.components.v1.iframe(google_form_url, height=800, scrolling=True)
    else:
        st.info("Paste your Google Form link above to display it here.")
